...
# clean removes the copied output artifact and the wit-bindgen-generated
# src/bindings.rs (analogous to Python's bindings/ and JavaScript's
# transpiled/ - a build output, regenerated by the next `build`). The cargo
# build cache under $CARGO_TARGET_DIR is preserved (constitution.md §4: clean
# removes outputs, not caches/installed dependencies) - it is only removed by
# `purge`.
cmd_clean() {
  rm -f task-component.wasm src/bindings.rs
}
